...
const argv = yargs([...envArgs, ...hideBin(process.argv)])
  ...
  .options({
    source: {
      ...
      type: "string",
      description:
        "Source folder to archive (defaults to current working directory)",
    ...
    output: {
      ...
      type: "string",
      description:
        "Output file name (defaults to txtzip.md in the current directory)",
    ...
    "prefix-tree": {
      ...
      type: "boolean",
      description:
        "Prefix the output with a tree-like structure of included files",
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        for (const pattern of includePatterns) {
          ...
          const matchPath = isRecursive
            ? relativePath
            : path.relative(resolvedSourceFolder, fullPath);
  ...
  try {
    ...
    if (prefixTree) {
      const relativePaths = allFiles.map((file) =>
        path.relative(resolvedSourceFolder, file)
      );
      ...
      const treeStringWithSpacing =
        "## File Structure\n\n```plaintext\n" + treeString + "\n```\n";
      const treeStringSize = Buffer.byteLength(treeStringWithSpacing, "utf8");

      if (
        maxChunkSize > 0 &&
        currentChunkSize + treeStringSize > maxChunkSize
      ) {
    ...
    for (const file of allFiles) {
      ...
      if (fileStats.isFile()) {
        ...
        if (!isBinaryFile(contentBuffer)) {
          ...
          let totalFileContentSize = Buffer.byteLength(
            totalFileContent,
            "utf8"
          );
          ...
          while (contentPointer < totalFileContent.length) {
            let remainingChunkSpace =
              maxChunkSize > 0 ? maxChunkSize - currentChunkSize : Infinity;
            let remainingContentLength =
              totalFileContent.length - contentPointer;
            let sliceLength = Math.min(
              remainingChunkSpace,
              remainingContentLength
            );

            // Ensure we don't cut in the middle of a multi-byte character
            let contentSlice = totalFileContent.substr(
              contentPointer,
              sliceLength
            );
            ...
            if (
              ...
              while (
                Buffer.byteLength(contentSlice, "utf8") > remainingChunkSpace &&
                sliceLength > 0
              ) {
                sliceLength--;
                contentSlice = totalFileContent.substr(
                  contentPointer,
                  sliceLength
                );
              }
    ...
    for (let i = 0; i < outputFilesContent.length; i++) {
      const outputFilePath =
        outputFilesContent.length > 1
          ? getOutputFilePath(i + 1)
          : resolvedOutputFile;
    ...
    console.log(
      `Markdown archive created successfully with ${outputFilesContent.length} file(s).`
    );
  } catch (error) {
